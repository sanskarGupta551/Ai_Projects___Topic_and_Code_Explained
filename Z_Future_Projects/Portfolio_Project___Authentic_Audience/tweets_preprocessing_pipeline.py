'''
This module performs Text Preprocessing on Tweets for Sentiment Analysis
'''
...
from keras.preprocessing.text import text_to_word_sequence ##Tokenization

pd.options.mode.chained_assignment=None ##Supresses both warning and error
...
## RegEx for removing Irrelevant Items
twitter_handle = r"@(\w+)"  
...
## Chat Words Set and Dictionary
chat_words_set = {
                   'KISS', 'M8', 'ROFLOL', 'L8R', 'PRW', 'IMHO', 'ROFL', 'BBL', 'WB', 
                   'BRB', 'FWIW', 'CYA', 'NRN', 'THX', 'WUF', 'BBS', 'U', 'ROTFLMAO', 
                   'ATK', 'SK8', 'LDR', 'IMO', 'A3', 'BRT', 'B4N', 'FAQ', 'OIC', 'PRT', 
                   'AFAIK', 'FC', 'IC', 'ASL', 'MTE', 'U2', '7K', 'FYI', 'GAL', 'LOL', 
                   'BFN', 'STATS', 'WTG', 'GMTA', 'TTFN', 'CUL8R', 'GN', 'GG', 'G9', 
                   'TTYL', 'PITA', 'GR8', 'ICQ', 'ASAP', 'U4E', 'WTF', 'W8', 'BAK', 
                   'ILU', 'AFK', 'IOW', 'ATM', 'IRL', 'CU', 'LTNS', 'LMAO', 'B4', 'BTW',
                   'OMG'}
chat_words_map_dict = {
                        ...
                        'BBS': 'Be Back Soon', 'BFN': 'Bye For Now', 'B4N': 'Bye For Now', 
                        'BRB': 'Be Right Back', 'BRT': 'Be Right There', 'BTW': 'By The Way',
                        'B4': 'Before', 'CU': 'See You', 'CUL8R': 'See You Later', 
                        ...
                        'ILU': 'ILU: I Love You', 'IMHO': 'In My Honest/Humble Opinion', 
                        ...
                        'M8': 'Mate', 'NRN': 'No Reply Necessary', 'OIC': 'Oh I See', 'OMG': 'Oh My God',
                        ...
                        'U4E': 'Yours For Ever', 'WB': 'Welcome Back', 'WTF': 'What The F...', 
                        'WTG': 'Way To Go!', 'WUF': 'Where Are You From?', 'W8': 'Wait...', '7K': 'Sick:-D Laugher'}

## Irrelevant Stopwords Set
irrelevant_stopwords_set = {
                            'after', 'into', 'all', 'my', 'an', 'are', 'our', 'below', 
                            'how', 'now', 'until', 'more', 'does', 'once', 'these', 
                            'which', 'while', 'if', 'we', 'them', 'who', 'each', 'with', 
                            'there', 'by', 'before', 'on', 'again', 'during', 'his', 'herself', 
                            'be', 'both', 'when', 'further', 'other', 'was', 'of', 'very', 'up', 
                            'yourselves', 'whom', 'their', 'those', 'do', 'where', 'some', 'me', 
                            'as', 'same', 'out', 'that', 'what', 'yours', 'only', 'so', 'being', 
                            'is', 'most', 'can', 'why', 'then', 'doing', 'such', 'having', 'about', 
                            'just', 'the', 'you', 'has', 'over', 'and', 'for', 'ours', 'did', 
                            'here', 'because', 'should', 'it', 'am', 'in', 'themselves', 'through', 
                            'under', 'than', 'myself', 'to', 'ourselves', 'have', 'any', 'your', 
                            'or', 'at', 'she', 'been', 'they', 'theirs', 'its', 'i', 'were', 'few', 
                            'itself', 'too', 'between', 'but', 'yourself', 'from', 'down', 'her', 
                            'off', 'this', 'him', 'a', 'will', 'above', 'himself', 'hers', 'he', 'had', 'own'}
...
## Tweets Preprocessing Sentiment Analysis
def tweets_preprocessing_sentiment_analysis(text):

    ## Removing Irrelevant Items
    text = re.sub(twitter_handle, "", text)
    ...
    ## Chat Slang Word Conversion
    new_tokens = []
    for w in tokens:
        if w.upper() in chat_words_set:
            new_tokens.append(chat_words_map_dict[w.upper()])
        else:
            new_tokens.append(w)
    ...
    ## Removing irrelevant stopwords
    denoised_text = []
    for w in text.split():
        if w not in irrelevant_stopwords_set:
            denoised_text.append(w)

    ## Return text
    text = " ".join(denoised_text)
    return text
